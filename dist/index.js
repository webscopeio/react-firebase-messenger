...
var transformMessagesToStoreInDB = function transformMessagesToStoreInDB(userId) {
  return compose(toPairs, map(compose(evolve({
...
var loadMoreMessagesListTransform = function loadMoreMessagesListTransform(participants) {
  return compose(
  // R.reverse,
  sortBy(prop('createdAt')), map(compose(dissoc('userId'), evolve({
    createdAt: unixToJSDate
  }), createUserObject(participants))), values);
};

// handling one single message which is got from firebase
var listnerSingleMessageTransform = function listnerSingleMessageTransform(messageSnippet, participants) {
  return compose(dissoc('userId'), evolve({
    createdAt: unixToJSDate
  }), assoc('_id', messageSnippet.key), createUserObject(participants));
};
...
var toFlatList = function toFlatList(userChats) {
  return compose(reverse, sortBy(prop('lastMessageCreatedAt')), values, function (item) {
    ...
    keys$$1.forEach(function (key) {
      return item[key] = compose(assoc('chatId', key))(item[key]);
    });
...
var ChatListProvider = function (_React$Component) {
  ...
  function ChatListProvider() {
    ...
    classCallCheck(this, ChatListProvider);
  ...
  createClass(ChatListProvider, [{
    key: 'render',
    value: function render() {
      var _props = this.props,
          ChatList = _props.component,
          rest = objectWithoutProperties(_props, ['component']);


      return React.createElement(ChatList, _extends({
...
var addUserToChat = function addUserToChat(firebaseDB, uid, chatId) {
  var upp = new Promise(function (resolve) {
    firebaseDB.child('chat-metadata/' + chatId).on('value', function (snap) {
      resolve(snap.val());
    });
  });
  ...
  upp.then(function (chatData) {
    ...
    var entireUpdate = (_entireUpdate = {}, defineProperty(_entireUpdate, 'chat-metadata/' + chatId + '/users/' + uid, true), defineProperty(_entireUpdate, 'user-chats/' + uid + '/' + chatId + '/lastMessageCreatedAt', toUnixTimestamp(chatData.lastMessageCreatedAt)), _entireUpdate);
...
var checkForChatExistence = function checkForChatExistence(firebaseDB, theUserId, uid, eventId) {
  return new Promise(function (resolve) {
    firebaseDB.child('chat-metadata').orderByChild('users/' + theUserId).startAt(true).on('value', function (snap) {
...
var getGroupChatsByEvent = function getGroupChatsByEvent(firebaseDB, eventId) {
  return new Promise(function (resolve) {
    firebaseDB.child('chat-metadata').orderByChild('eventId').equalTo(eventId).on('value', function (snap) {
      var chats = snap.val();
      firebaseDB.child('chat-metadata').off();
      resolve(chats);
    });
...
var ChatProviderWrapper = function ChatProviderWrapper(firebaseDB, ComposedComponent) {
  var ChatProvider = function (_React$Component) {
    ...
    function ChatProvider() {
      ...
      classCallCheck(this, ChatProvider);

      for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }

      return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = ChatProvider.__proto__ || Object.getPrototypeOf(ChatProvider)).call.apply(_ref, [this].concat(args))), _this), _this.state = chatDefaultState, _this.resetChat = function (callback) {
      ...
      }, _this.createNewChat = function (_ref3) {
        var newChatId = _ref3.newChatId,
        ...
        var uid = _ref6.uid,
            prevMessages = _ref6.prevMessages;
      ...
      }, _this.checkForChat = function (_ref7) {
        var theUserId = _ref7.theUserId,
            uid = _ref7.uid,
            eventId = _ref7.eventId;

        return checkForChatExistence(firebaseDB, theUserId, uid, eventId);
      }, _this.getChatParticipantsDetails = function () {
        var _ref8 = asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(participants) {
          var allChatsParticipants;
              ...
              switch (_context.prev = _context.next) {
                case 0:
                  ...
                  return Promise.all(keys(participants).map(function (participantId) {
                    return new Promise(function (resolve) {
                      return usersRef(firebaseDB, participantId).once('value', function (userSnapshot) {
                        var userInfo = userSnapshot.val();
                        var userData = _extends({}, userInfo, { uid: participantId });
...
exports.ChatListProvider = ChatListProvider;
exports.ChatWindowsProvider = ChatProviderWrapper;
exports.createEmptyChat = createEmptyChat;
exports.addUserToChat = addUserToChat;
exports.getGroupChatsByEvent = getGroupChatsByEvent;
